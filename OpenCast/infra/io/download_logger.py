...
class DownloadLogger(object):
    ...
    def _log_download_info(self, d):
        ...
        self._logger.info(
            f"{filename} | {self._format_ratio(d)} | {self._format_speed(d)}"
        )

    def _log_download_error(self, d):
        filename = d.get("filename", "unknown")
        self._logger.error(f"error downloading {filename}: {d}")

    def _log_download_finished(self, d):
        filename = d.get("filename", "unknown")
        total = d.get("total_bytes", 0)
        self._logger.info(f"finished downloading {filename} ({size(total)})")
