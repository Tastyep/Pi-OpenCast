from OpenCast.config import Downloader, Server, VideoPlayer, config
...
class ConfigTest(TestCase):
    def test_categories(self):
        ...
        self.assertIsInstance(config["Subtitle"], Downloader)
    ...
    def test_parse_downloader_config(self):
        config.load_from_dict({"Downloader": {"output_directory": "/Video"}})

        dl = config["Downloader"]
        self.assertEqual(dl.output_directory, "/Video")

    def test_parse_subtitle_config(self):
        config.load_from_dict({"Subtitle": {"language": "spa"}})

        subtitle = config["Subtitle"]
        self.assertEqual(subtitle.language, "spa")

    def test_parse_server_config(self):
        config.load_from_dict({"Server": {"host": "host", "port": 42}})
    ...
    def test_parse_quoted_string(self):
        config.load_from_dict({"Server": {"host": '"host"'}})
